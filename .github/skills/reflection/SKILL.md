...
| `{{PHASE_NUMBER}}` | Read from `.maid/state.json` → `current_phase` |
| `{{PHASE_NAME}}` | Read from `.maid/state.json` → `phase_name` |
...
- `{{STATE_JSON}}` - Content of `.maid/state.json`
- `{{CONTEXT_JSON}}` - Content of `.maid/context.json`
