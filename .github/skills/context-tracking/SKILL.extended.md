...
Maintain real-time work context in `.maid/context.json` for seamless session continuity, integrated with phase enforcement and quality feedback systems.
...
│                                                                 │
│  github MUST maintain context at ALL times:                     │
│                                                                 │
│  1. READ context before starting any work                       │
│  2. UPDATE context on every significant change                  │
│  3. SYNC with phase state (.maid/state.json)                     │
│  4. INTEGRATE with feedback system (/MAID end)                   │
...
| `feedback_collected` | /MAID end completed | Rating |
...
│       │                                                         │
│       ├── Yes ──▶ Prompt: "/MAID end for phase feedback"         │
│       │                                                         │
│       ▼                                                         │
│  Check: Sessions since feedback >= 3?                           │
│       │                                                         │
│       ├── Yes ──▶ Remind: "Consider /MAID end for feedback"      │
│       │                                                         │
│       ▼                                                         │
...
| Condition | Reminder |
|-----------|----------|
| Phase ending | "Run /MAID end before advancing to next phase" |
| 3+ sessions without feedback | "Consider /MAID end to share what worked" |
| 5+ tasks completed | "Good time for /MAID end checkpoint" |
...
| `/MAID end` | Update feedback tracking, save context |
| `/MAID improve` | Reset feedback counters |
...
When threshold reached, github displays:
...
Can be overridden in `~/.maid/config.yaml`:
